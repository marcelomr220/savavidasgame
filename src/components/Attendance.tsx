...
export default function Attendance({ user, onUpdateUser }: { user: User, onUpdateUser?: () => void }) {
  ...
  const handleCheckin = async (code: string) => {
    ...
    try {
      const points = await checkIn(user.id, code);
      setStatus({ type: 'success', message: `Presença confirmada! +${points} pontos.` });
        ...
        <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm">
          ...
          <div className="flex flex-col gap-3">
            <input
              ...
              placeholder="CÓDIGO"
              className="w-full px-4 py-4 bg-stone-50 border border-stone-200 rounded-2xl text-center font-mono font-bold text-2xl tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 uppercase"
            />
            <button
              onClick={() => handleCheckin(manualCode)}
              disabled={manualCode.length < 6}
              className="w-full py-4 bg-stone-900 text-white rounded-2xl font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-stone-800 transition-all shadow-lg shadow-stone-200"
            >
              Validar Código
            </button>
          </div>
