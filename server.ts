...
try {
  if (db) {
    ...
    CREATE TABLE IF NOT EXISTS game_player_tasks (
      id TEXT PRIMARY KEY,
      player_id TEXT,
      task_id TEXT,
    ...
    CREATE TABLE IF NOT EXISTS game_events (
      id TEXT PRIMARY KEY,
      match_id TEXT,
      type TEXT NOT NULL, -- 'kill', 'report', 'meeting', 'sabotage'
      created_by TEXT,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
async function startServer(app: any) {
  ...
  // --- SOCIAL DEDUCTION GAME API ---
  // --- Helper Functions for Game Sync ---
  async function ensureUserInSQLite(userId: number) {
    if (!db) return;
    const sqliteUser = db.prepare("SELECT id FROM users WHERE id = ?").get(userId);
    if (!sqliteUser && supabase) {
      const { data: sbUser } = await supabase.from('users').select('*').eq('id', userId).single();
      if (sbUser) {
        db.prepare("INSERT OR IGNORE INTO users (id, name, email, password, avatar, role, team_id, points, level, streak) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
          .run(sbUser.id, sbUser.name, sbUser.email, sbUser.password || 'sb_synced', sbUser.avatar, sbUser.role || 'user', sbUser.team_id, sbUser.points || 0, sbUser.level || 1, sbUser.streak || 0);
      }
    }
  }

  async function ensureMatchInSQLite(matchId: string) {
    if (!db) return;
    const sqliteMatch = db.prepare("SELECT id FROM game_matches WHERE id = ?").get(matchId);
    if (!sqliteMatch && supabase) {
      const { data: sbMatch } = await supabase.from('game_matches').select('*').eq('id', matchId).single();
      if (sbMatch) {
        db.prepare("INSERT OR IGNORE INTO game_matches (id, name, status, created_at) VALUES (?, ?, ?, ?)")
          .run(sbMatch.id, sbMatch.name, sbMatch.status, sbMatch.created_at);
      }
    }
  }

  app.get("/api/games/social-deduction/matches", async (req, res) => {
    try {
      if (supabase) {
        const { data, error } = await supabase
          .from('game_matches')
      ...
      const matches = db.prepare("SELECT * FROM game_matches ORDER BY created_at DESC").all();
      res.json(matches);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/games/social-deduction/matches", async (req, res) => {
    const { name } = req.body;
    const id = Math.random().toString(36).substring(2, 15);
    try {
      // SQLite
      db.prepare("INSERT INTO game_matches (id, name, status) VALUES (?, ?, 'waiting')").run(id, name);
      
      // Supabase
      if (supabase) {
        await supabase.from('game_matches').insert([{ id, name, status: 'waiting' }]);
      }
      
      const match = db.prepare("SELECT * FROM game_matches WHERE id = ?").get(id);
      res.json(match);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.get("/api/games/social-deduction/players/:matchId", async (req, res) => {
    ...
    try {
      ...
      const players = db.prepare("SELECT * FROM game_players WHERE match_id = ?").all(matchId);
      res.json(players);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.get("/api/games/social-deduction/player-tasks/:playerId", async (req, res) => {
    const { playerId } = req.params;
    try {
      if (supabase) {
        const { data, error } = await supabase
          .from('game_player_tasks')
          .select('*, task:game_tasks(*)')
          .eq('player_id', playerId);
        if (!error) return res.json(data);
      }
      
      const playerTasks = db.prepare(`
        SELECT gpt.*, gt.title, gt.description, gt.points
        FROM game_player_tasks gpt
        JOIN game_tasks gt ON gpt.task_id = gt.id
        WHERE gpt.player_id = ?
      `).all(playerId);
      
      const formatted = playerTasks.map(pt => ({
        ...pt,
        task: {
          id: pt.task_id,
          title: pt.title,
  ...
  app.get("/api/games/social-deduction/votes/:meetingId", async (req, res) => {
    ...
    try {
      ...
      const votes = db.prepare("SELECT * FROM game_votes WHERE meeting_id = ?").all(meetingId);
      res.json(votes);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/games/social-deduction/join", async (req, res) => {
    const { matchId, userId, name, avatar } = req.body;
    const id = Math.random().toString(36).substring(2, 15);
    try {
      await ensureUserInSQLite(userId);
      await ensureMatchInSQLite(matchId);

      // SQLite
      db.prepare("INSERT INTO game_players (id, user_id, match_id, name, avatar, is_alive) VALUES (?, ?, ?, ?, ?, 1)").run(id, userId, matchId, name, avatar);
      
      // Supabase
      if (supabase) {
        await supabase.from('game_players').insert([{ id, user_id: userId, match_id: matchId, name, avatar, is_alive: 1 }]);
      }
      
      const player = db.prepare("SELECT * FROM game_players WHERE id = ?").get(id);
      res.json(player);
    } catch (err: any) {
  ...
  app.post("/api/games/social-deduction/start", async (req, res) => {
    const { matchId, players } = req.body; // players is array of { id, role }
    try {
      await ensureMatchInSQLite(matchId);

      // Update match status
      db.prepare("UPDATE game_matches SET status = 'in_progress' WHERE id = ?").run(matchId);
      
      // Update player roles
      const updatePlayer = db.prepare("UPDATE game_players SET role = ? WHERE id = ?");
      const transaction = db.transaction((players) => {
        for (const p of players) {
          updatePlayer.run(p.role, p.id);
        }
      });
      transaction(players);
      ...
      // Supabase
      if (supabase) {
        await supabase.from('game_matches').update({ status: 'in_progress' }).eq('id', matchId);
        for (const p of players) {
          await supabase.from('game_players').update({ role: p.role }).eq('id', p.id);
        }
      }
  ...
  app.post("/api/games/social-deduction/kill", async (req, res) => {
    const { matchId, killerId, victimId } = req.body;
    const eventId = Math.random().toString(36).substring(2, 15);
    try {
      await ensureMatchInSQLite(matchId);

      // Update victim status
      db.prepare("UPDATE game_players SET is_alive = 0 WHERE id = ?").run(victimId);
      
      // Record event
      db.prepare("INSERT INTO game_events (id, match_id, type, created_by) VALUES (?, ?, 'kill', ?)").run(eventId, matchId, killerId);
      
      // Supabase
      if (supabase) {
        await supabase.from('game_players').update({ is_alive: 0 }).eq('id', victimId);
        await supabase.from('game_events').insert([{ id: eventId, match_id: matchId, type: 'kill', created_by: killerId }]);
      }
  ...
  app.post("/api/games/social-deduction/report", async (req, res) => {
    const { matchId, reporterId } = req.body;
    const eventId = Math.random().toString(36).substring(2, 15);
    try {
      await ensureMatchInSQLite(matchId);
      db.prepare("INSERT INTO game_events (id, match_id, type, created_by) VALUES (?, ?, 'report', ?)").run(eventId, matchId, reporterId);
      
      if (supabase) {
        await supabase.from('game_events').insert([{ id: eventId, match_id: matchId, type: 'report', created_by: reporterId }]);
      }
  ...
  app.post("/api/games/social-deduction/vote", async (req, res) => {
    const { matchId, voterId, votedPlayerId, meetingId } = req.body;
    const voteId = Math.random().toString(36).substring(2, 15);
    try {
      await ensureMatchInSQLite(matchId);
      db.prepare("INSERT INTO game_votes (id, match_id, voter_id, voted_player_id, meeting_id) VALUES (?, ?, ?, ?, ?)").run(voteId, matchId, voterId, votedPlayerId, meetingId);
      
      if (supabase) {
        await supabase.from('game_votes').insert([{ id: voteId, match_id: matchId, voter_id: voterId, voted_player_id: votedPlayerId, meeting_id: meetingId }]);
      }
  ...
  app.post("/api/games/social-deduction/complete-task", async (req, res) => {
    const { playerTaskId, matchId } = req.body;
    try {
      await ensureMatchInSQLite(matchId);
      db.prepare("UPDATE game_player_tasks SET completed = 1 WHERE id = ?").run(playerTaskId);
      
      if (supabase) {
        await supabase.from('game_player_tasks').update({ completed: 1 }).eq('id', playerTaskId);
      }
  ...
  app.post("/api/games/social-deduction/end", async (req, res) => {
    const { matchId, winnerRole } = req.body;
    try {
      await ensureMatchInSQLite(matchId);
      db.prepare("UPDATE game_matches SET status = 'finished', winner_role = ? WHERE id = ?").run(winnerRole, matchId);
      
      if (supabase) {
        await supabase.from('game_matches').update({ status: 'finished', winner_role: winnerRole }).eq('id', matchId);
      }
